...
def iterate(mode, args, loader, model, optimize, logger, epoch):
    ...
    if mode == 'train':
        model.train()
        lr = helper.adjust_learning_rate(args.lr, optimize, epoch)
    else:
        model.eval()
        lr = 0
...
def main():
    ...
    if not is_eval:
        ...
        train_loader=paddle.io.DataLoader(train_dataset,batch_size=args.batch_size,shuffle=True,num_workers=args.workers)
