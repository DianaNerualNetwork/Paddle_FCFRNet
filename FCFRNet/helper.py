...
import csv
import vis_utils
from metric import Result

fieldnames = [
    'epoch', 'rmse', 'photo', 'mae', 'irmse', 'imae', 'mse','rmse2', 'irmse1', 'imae2', 'imae1', 'irmse2', 'mae1', 'rmse1', 'mae2', 'absrel', 'lg10',
...
class logger:
    ...
    def conditional_print(self, split, i, epoch, lr, n_set, blk_avg_meter,
        ...
        if (i + 1) % self.args.print_freq == 0:
            ...
            print(
                ...
                'RMSE1={blk_avg.rmse1:.2f}({average.rmse1:.2f}) '
                'MAE1={blk_avg.mae1:.2f}({average.mae1:.2f}) '
                'iRMSE1={blk_avg.irmse1:.2f}({average.irmse1:.2f}) '
                'iMAE1={blk_avg.imae1:.2f}({average.imae1:.2f})\n\t'
                'RMSE2={blk_avg.rmse2:.2f}({average.rmse2:.2f}) '
                'MAE2={blk_avg.mae2:.2f}({average.mae2:.2f}) '
                'iRMSE2={blk_avg.irmse2:.2f}({average.irmse2:.2f}) '
                'iMAE2={blk_avg.imae2:.2f}({average.imae2:.2f})\n\t'
    ...
    def rank_conditional_save_best(self, mode, result, epoch):
        ## args: mode(train val) result()  epoch:int
    ...
    def conditional_save_pred(self, mode, i, pred, epoch):
        ...
        if ("test" in mode or mode == "eval" ) and self.args.save_pred :
            ...
            img = paddle.squeeze(pred.cpu()).numpy()
